...
func dropDisabledAssignedCpuset(podSpec, oldPodSpec *api.PodSpec) {
	...
	// If a volume only contains assigned.cpuset items, drop the entire volume
	var retainedVolumes []api.Volume
	for i := range podSpec.Volumes {
		...
		if vol.DownwardAPI != nil {
			...
			if len(filteredItems) == 0 {
				// If no items left, drop the entire DownwardAPI volume source
				vol.DownwardAPI = nil
				// Check if the volume is now empty (no volume source set)
				if vol.VolumeSource == (api.VolumeSource{}) {
					// Skip this volume (don't retain it)
					continue
				}
			} else {
		...
		retainedVolumes = append(retainedVolumes, *vol)
	}
	podSpec.Volumes = retainedVolumes
}
