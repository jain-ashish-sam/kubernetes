...
func Test_dropDisabledAssignedCpuset(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "feature disabled, only current pod uses assigned.cpuset - single item (should drop entire Volume)",
			enabled: false,
			oldPodSpec: &api.PodSpec{
				Volumes: nil,
			},
		...
		{
			name:    "feature disabled, mixed volumes: downward-api with only assigned.cpuset + emptyDir",
			enabled: false,
			oldPodSpec: &api.PodSpec{
				Volumes: nil,
			},
